...
// mutating request (POST/PUT/DELETE), so any write — by any admin — fans out to
// every open dashboard within one round-trip.
type wsHub struct {
...
// header on a WS handshake, so the master-auth bearer token is passed as a query
// parameter and verified the same way requireAuth verifies the header.
func (h *Handler) ws(w http.ResponseWriter, r *http.Request) {
	claims, err := h.verify.Verify(r.URL.Query().Get("token"))
	if err != nil || !claims.CanAccess(deptCode) {
